...
쉽게 빠지는 오해 중 하나는 실제 능력 결함을 진단하기 전에 합성 인스트럭션부터 적용하는 것입니다. SFT는 행동과 사실 패턴을 모두 학습시킬 수 있지만, 사실이 자주 바뀌거나 출처가 필요한 경우에는 검색을 대신하기 어렵고, 작은 인스트럭션 세트로 넓은 도메인 표현 격차를 고치기도 어렵습니다.

변하거나 출처가 필요한 지식에는 검색 증강 생성, 지도된 과업 행동에는 SFT, 넓은 분포 적응에는 지속 사전 학습이라는 세 가지 통제 기준선을 비교해야 합니다. 한 방법은 “무엇”을, 다른 방법은 “어떻게”를 가르친다는 구호가 아니라 홀드아웃 과업 결과로 선택합니다.

베이스·인스트럭션 체크포인트 선택, 리플레이 혼합, 토크나이저 변경, 파일럿 확장, 정확한 재개, 망각 평가, 롤백을 포함한 전체 실무 절차는 [8.5 지속 사전 학습 및 도메인 적응](/ko/chapter-8/continued-pretraining-and-domain-adaptation)에서 다룹니다.
