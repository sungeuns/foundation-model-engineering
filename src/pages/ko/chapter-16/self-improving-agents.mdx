...
**근거 스냅샷: 2026년 9월 15일.** 아래 시스템은 연구 프로토타입 또는 제약된 제품이지, 제약 없는 재귀적 자기 개선의 증거가 아닙니다. 보고된 벤치마크 향상은 특정 예산에서 실행한 모델·스캐폴드·평가기의 결합 결과입니다.
...
### 통계적 함정: 가장 운이 좋은 후보를 고르는 문제

시스템이 $K$개의 변형을 만든다고 합시다. 후보 $i$의 개발셋 추정치는 다음과 같습니다.
...
여기서 $J(\theta_i)$는 관측할 수 없는 실제 배포 품질이고, $\epsilon_i$는 측정 잡음입니다. 선택 전에는 모든 추정치가 불편향이어도 가장 높은 값을 고른 뒤의 승자 추정치는 낙관적으로 치우칩니다. 후보를 많이 탐색할수록 양의 잡음을 뽑을 기회가 늘기 때문입니다. 세대마다 같은 벤치마크를 재사용하면 이 **winner's curse** 가 적응적 벤치마크 과적합으로 누적됩니다.

방어 가능한 승격 규칙은 세 집합을 분리합니다.

1. 반복해서 조회할 수 있는 **탐색셋**
2. 승격 결정에 한 번 사용하는 **봉인 홀드아웃**
3. 미리 정한 롤백 임계값으로 실제 트래픽을 측정하는 **배포 후 카나리**

실질적으로 의미 있는 최소 향상 폭을 $\delta$라고 하면, 보수적인 게이트는 점 추정치가 아니라 불확실성 경계를 비교합니다.
...
홀드아웃은 버전 관리와 접근 통제를 적용하고, 반복 노출된 뒤에는 폐기해야 합니다. 그렇지 않으면 조용히 또 하나의 개발셋이 됩니다.

### Interactive: 개발셋 승자가 게이트에서 탈락하는 이유

개발 과업 수를 작게 유지한 채 후보 변형 수를 늘려 보세요. 주황색 개발셋 추정치는 선택에 사용되지만, 파란색 홀드아웃 추정치는 탐색 루프에 공개되지 않습니다. 이어서 배포 분포 이동을 키우면 로컬에서 성공한 스캐폴드가 프로덕션에서는 퇴행할 수 있는 이유를 볼 수 있습니다.

<SelfImprovementGateVisualizer lang="ko" client:load />
...
논문은 명시된 하네스 안에서 큰 향상을 보고합니다. 해당 SWE-bench 설정에서는 20.0%에서 50.0%로, Polyglot에서는 14.2%에서 30.7%로 향상되었습니다. 이는 베이스 모델 자체의 아키텍처 독립적 향상이 아니라 전체 DGM 시스템의 결과입니다. 발견된 변경도 추상적인 prompt trick이 아니라 더 나은 editing tools, long-context management, peer-review mechanisms 같은 구체적인 engineering improvement였습니다.
...
승인된 변형은 단순히 점수가 높을 뿐 아니라 재현 가능하고 되돌릴 수 있어야 합니다. 최소 기록에는 부모 아티팩트 해시, 정확한 diff, 베이스 모델 스냅샷, 도구·권한 스키마, 평가기 버전, 과업 ID, seed, 토큰·도구·지연 시간 예산, 신뢰구간, 알려진 slice 퇴행, 검토자 신원이 포함됩니다. 이전 번들은 last-known-good 버전으로 보존하고, 카나리 트래픽을 시작하기 전에 롤백 조건을 정합니다.

후보가 자기 acceptance test를 같은 신뢰 영역에서 수정하게 해서는 안 됩니다. 평가기, 샌드박스, 권한 또는 홀드아웃 매니페스트 변경은 별도의 검토 경로를 거쳐야 합니다. 즉 에이전트는 측정법 변경을 제안할 수 있지만, 같은 실행에서 자를 다시 쓰고 자신이 더 커졌다고 선언할 수는 없습니다.
