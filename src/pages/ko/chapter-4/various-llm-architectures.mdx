...
| Kimi K3 | hosted model 공개; full weights와 technical report는 7월 27일 예정 | 2.8T parameter, native multimodality, 1M context, Kimi Delta Attention, Attention Residuals, Stable LatentMoE, 16-of-896 expert routing [[11]](#ref-11) | weight file, config 차원, data mixture, 전체 training recipe |
| Qwen3.7 | agent 중심 Qwen 세대로 공개 | 1M-context 제품 설정과 long-horizon agent 사례 [[12]](#ref-12) | 이 정보는 Qwen3.8 내부 구조의 증거가 아님 |
| Qwen3.8-Max-Preview | 2.4T hosted preview; open weights 공개 예정 | parameter 규모와 제품 제공 경로 [[13]](#ref-13) | weights, model card, architecture config, corpus recipe, 상세 post-training report |
...
K3는 recurrent/linear 경로인 Kimi Delta Attention, 레이어 사이 표현을 attention 방식으로 섞는 Attention Residuals, Stable LatentMoE를 결합합니다. Router는 896개 expert 중 16개를 선택합니다. 이는 **expert pool의 약 1.8%**이지, 전체 parameter의 1.8%만 활성화된다는 증거가 아닙니다. Embedding, attention, shared component, expert 크기를 알아야 진짜 active-parameter ratio를 계산할 수 있습니다. Moonshot은 K2 대비 2.5배 높은 scaling efficiency를 제시하지만, 그 정의를 감사할 technical report는 다음 날 공개 예정입니다 [[11]](#ref-11).
...
### Qwen3.8: ‘아직 없음’도 기술적 사실이다

Qwen3.8-Max-Preview는 근거 규율을 가르쳐 준다는 점에서 포함할 가치가 있습니다. Alibaba는 2.4T preview와 향후 open weights를 발표했지만, 현재 expert 수, attention layout, data mixture, training pipeline을 뒷받침할 공개 artifact는 없습니다 [[13]](#ref-13). Qwen3.7의 세부 정보를 후속 모델에 조용히 이식해서는 안 됩니다. Artifact가 올라오기 전까지 기술적으로 올바른 아키텍처 표기는 추정치가 아니라 **미공개**입니다.
