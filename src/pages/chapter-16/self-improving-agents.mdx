...
import SelfImprovementGateVisualizer from '../../components/chapter-16/SelfImprovementGateVisualizer.tsx';
...
**Evidence snapshot: September 15, 2026.** The systems below are research prototypes or constrained products, not evidence of unconstrained recursive self-improvement. Reported benchmark gains belong to a model-plus-scaffold-plus-evaluator configuration under a particular budget.
...
### The statistical trap: selecting the luckiest candidate

Suppose the system generates $K$ variants. For variant $i$, the development estimate is

$$
\hat{J}_{\mathrm{dev}}(\theta_i) = J(\theta_i) + \epsilon_i,
\qquad
i^* = \arg\max_{i \in \{1,\ldots,K\}} \hat{J}_{\mathrm{dev}}(\theta_i),
$$

where $J(\theta_i)$ is the unobserved deployment quality and $\epsilon_i$ is measurement noise. Even when every estimate is unbiased before selection, the winning estimate is optimistic after selection: searching more candidates gives the loop more chances to select a positive noise realization. Reusing the same benchmark in every generation turns that **winner's curse** into adaptive benchmark overfitting.

A defensible promotion rule separates three sets:

1. a **search set** that may be queried repeatedly;
2. a **sealed holdout** used once for a promotion decision; and
3. a **post-deployment canary** that measures real traffic under rollback thresholds.

For a required practical margin $\delta$, a conservative gate compares uncertainty bounds rather than point estimates:

$$
\operatorname{LCB}_{95\%}(\hat{J}_{\mathrm{holdout}}(\theta_{i^*}))
>
\operatorname{UCB}_{95\%}(\hat{J}_{\mathrm{holdout}}(\theta_{0})) + \delta.
$$

The holdout must be versioned, access-controlled, and retired after repeated exposure. Otherwise it silently becomes another development set.

### Interactive: why the development winner may fail the gate

Increase the number of candidate variants while keeping the development set small. The orange development estimate is used for selection; the blue holdout estimate is not visible to the search loop. Then increase deployment shift to see why a locally successful scaffold can still regress in production.

<SelfImprovementGateVisualizer client:load />
...
The paper reports substantial gains within its stated harness: from 20.0% to 50.0% on its SWE-bench setup and from 14.2% to 30.7% on Polyglot. These are results for the complete DGM system, not an architecture-independent increase in the base model's coding ability. The discovered changes are concrete engineering improvements: better editing tools, long-context management, and peer-review mechanisms.
...
### A production promotion contract

An accepted variant should be reconstructable and reversible, not merely higher-scoring. A minimal record includes the parent artifact hash, exact diff, base-model snapshot, tool and permission schema, evaluator version, task IDs, seeds, token/tool/latency budget, confidence interval, known slice regressions, and reviewer identity. Store the previous bundle as the last-known-good version and define rollback triggers before canary traffic begins.

Do not let the candidate modify its own acceptance test in the same trust domain. Changes to evaluators, sandboxes, permissions, or the holdout manifest require a separate review path. This creates a two-key system: the agent may propose a measurement change, but the same run cannot both rewrite the ruler and declare itself taller.
